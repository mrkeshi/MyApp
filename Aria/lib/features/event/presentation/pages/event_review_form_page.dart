...
class EventReviewFormSheet extends StatefulWidget {
  final int eventId;
  const EventReviewFormSheet({super.key, required this.eventId});

  @override
  State<EventReviewFormSheet> createState() => _EventReviewFormSheetState();
}

class _EventReviewFormSheetState extends State<EventReviewFormSheet> {
  int rating = 0;
  ...
  void _showSnack({required String message, required Color bg}) {
    final snack = SnackBar(
      ...
      content: Center(child: Text(message, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600))),
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  ...
                  const Text('ثبت نظر جدید', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 16),
                  Expanded(
                                  ...
                                  Container(
                                    ...
                                    child: CircleAvatar(
                                      ...
                                      child: photoUrl.isEmpty
                                          ? Text(displayName.isNotEmpty ? displayName.characters.first : '؟', style: const TextStyle(color: Colors.white))
                                          : null,
                                    ),
                                  ),
                                ...
                                children: List.generate(5, (i) {
                                  ...
                                  return GestureDetector(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Icon(filled ? Icons.star : Icons.star_border, size: 28, color: primary),
                                    ),
                                  );
                                }),
                              ),
                  ...
                  Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: canSubmit
                            ? () async {
                          final c = context.read<EventsController>();
                          setState(() => submitting = true);
                          try {
                            final ok = await c.submitMyReview(
                              widget.eventId,
                              rating: rating,
                        ...
                        child: Text(submitting ? '...' : 'ثبت نظر', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                      ),
                    ),
                  ),
                ],
