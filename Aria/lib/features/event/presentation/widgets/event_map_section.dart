...
class EventMapSection extends StatelessWidget {
  ...
  const EventMapSection({super.key, required this.data});
  ...
  Map<String, double>? _extractLatLng(dynamic d) {
    double? lat, lng;
    if (d is Map) {
      lat = _toDouble(d['lat'] ?? d['latitude'] ?? (d['location']?['lat']) ?? (d['location']?['latitude'])
          ?? ((d['coordinates'] is List && d['coordinates'].length >= 2) ? d['coordinates'][0] : null));
      lng = _toDouble(d['lng'] ?? d['longitude'] ?? (d['location']?['lng']) ?? (d['location']?['longitude'])
  ...
  Widget build(BuildContext context) {
    ...
    final coords = _extractLatLng({'latitude': data.latitude, 'longitude': data.longitude});
    final label = _extractLabel({'title': data.title});
