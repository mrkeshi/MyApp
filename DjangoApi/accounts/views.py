...
from utilities.kavenegar import *
...
from .utils import normalize_phone_and_code, normalize_phone
...
def _calculate_resend_window(resend_count):

    if resend_count <= 1:
        return 0
    return 60 * (2 ** (resend_count - 1))
...
class RequestCodeView(APIView):
    ...
    def post(self, request):
        ...
        if recent_count >= 5:
            return Response(
                {"detail": "شما هر ساعت فقط می‌توانید ۵ درخواست دهید."},
        ...
        return Response(
            {
                "detail": "کد OTP ایجاد شد و به شماره شما ارسال شد.",
