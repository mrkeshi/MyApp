...
class EventReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EventReview
        fields = ("id","user","user_display","profile_image","rating","comment","created_at","created_at_text")
...
class EventDetailSerializer(EventSerializer):
    ...
    def get_reviews(self, obj):
        ctx = {"request": self.context.get("request"), "with_time": self.context.get("with_time", False)}
        return EventReviewSerializer(obj.reviews.select_related("user").all(), many=True, context=ctx).data
